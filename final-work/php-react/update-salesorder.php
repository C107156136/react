...
if (isset($data->OId)
    ...
    $insertUser = mysqli_query($db_connection, "UPDATE `salesorder` SET `EmpId`='$EId',`CustId`='$CId', `OrderDate`='$ODate', `Descript`='$Des' WHERE `OrderId`='$data->OId'");
    if ($insertUser) {
        echo json_encode(["success" => 1, "msg" => "User Not Inserted2!"]);
    } else {
        echo json_encode(["success" => 0, "msg" => "User Not Inserted3!"]);
    }
} else {
